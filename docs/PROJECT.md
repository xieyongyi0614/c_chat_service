...
| 文件                    | 说明                                                  |
| ----------------------- | ----------------------------------------------------- |
| `app-config.module.ts`  | 全局配置模块，基于 `@nestjs/config`，统一加载所有配置 |
| `modules/*.config.ts`   | 按领域拆分的配置（如 Redis / JWT / Upload / DB）      |
| `env/schema.ts`         | 环境变量校验（Joi），用于启动时验证配置合法性         |
| `env/loader.ts`（可选） | 多环境 `.env` 加载策略（如 `.env.development`）       |
| `index.ts`              | 统一导出配置                                          |
...
本项目采用 **NestJS 官方推荐的配置方案**：

- 使用 `registerAs` 定义模块化配置
- 使用 `ConfigType<typeof xxx>` 实现**类型安全**
- 使用 `validationSchema` 进行**启动时校验**
- 配置按领域拆分，避免集中式“巨型配置类”
...
| 模块               | 说明                            |
| ------------------ | ------------------------------- |
| `redis.config.ts`  | Redis 连接配置（Bull 队列复用） |
| `jwt.config.ts`    | JWT 鉴权配置                    |
| `upload.config.ts` | 文件上传相关配置                |
